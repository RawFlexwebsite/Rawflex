  ...
  if (inquiries.length === 0) {
    return (
      <div className="bg-panel rounded-2xl border border-cream-line p-12 text-center">
        <Mail className="w-12 h-12 text-ink/50 mx-auto mb-4" />
        <h3 className="text-lg font-bold text-ink">No inquiries yet</h3>
        <p className="text-sm text-ink/60 mt-1">When customers contact you, their messages will appear here.</p>
      </div>
    )
  }
    ...
    <div className="space-y-4">
      {inquiries.map((inquiry) => {
        ...
        return (
          <div 
            key={inquiry.id} 
            className={`bg-panel rounded-2xl border transition-all overflow-hidden ${
              isUnread ? 'border-orange-200 shadow-sm' : 'border-cream-line'
            }`}
          >
            {/* Header Row */}
            <div 
              className={`flex items-center justify-between p-5 cursor-pointer hover:bg-panel2 transition-colors ${
            ...
            >
              <div className="flex items-center gap-4 flex-1 min-w-0">
                <div className={`p-2 rounded-full ${isUnread ? 'bg-orange-100 text-orange-600' : 'bg-cream-deep text-ink/40'}`}>
                  {isUnread ? <MessageSquare className="w-5 h-5 fill-current" /> : <Mail className="w-5 h-5" />}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3">
                    <p className={`text-sm truncate ${isUnread ? 'font-bold text-ink' : 'font-medium text-ink/80'}`}>
                ...
                <div className="flex items-center gap-1" onClick={e => e.stopPropagation()}>
                  {isUnread && (
                    <button
                      ...
                      disabled={isPending}
                      className="p-2 text-ink/40 hover:text-green-600 hover:bg-green-50 rounded-lg transition-colors"
                      title="Mark as Read"
                    >
            ...
            {/* Expanded Content */}
            {isExpanded && (
              <div className="p-6 border-t border-cream-line bg-panel/50">
                <h4 className="text-xs font-bold text-ink/40 uppercase tracking-wider mb-3">Message</h4>
