...
export default function PakistaniEditBanner() {
  return (
    <section
      className="relative overflow-hidden pt-12 md:pt-16 pb-12 md:pb-16 border-y border-cream-line min-h-[450px] flex items-center bg-emerald-deep"
    >
        ...
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          ...
          <div className="flex flex-col items-center text-center py-10 lg:py-16 justify-center order-1 lg:order-2">
            ...
            <h2 className="mt-4 font-display font-bold text-4xl md:text-5xl text-ink leading-tight">
              The <span className="italic text-gold font-medium">Acid Wash</span> Edit
            </h2>

            <div className="w-24 h-[1px] bg-gold/30 my-6" />

            {/* Main Subtitle */}
            <p className="text-base md:text-lg text-ink/70 font-body max-w-[420px] leading-relaxed">
              One-of-one acid wash finishes, heavyweight cotton and a colour run
              that refuses to repeat. Limited pieces, no restocks.
            </p>

            {/* CTA Button */}
            <Link
              href="/shop?search=acid"
              className="mt-10 inline-flex items-center justify-center px-10 py-3.5 rounded-full border border-gold text-gold-light font-body font-semibold text-[15px] tracking-wide hover:bg-gold hover:text-black transition-all hover:scale-[1.02]"
            >
              Shop The Drop
            </Link>
          </div>
